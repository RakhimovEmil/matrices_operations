...
int main(int argc, char** argv) {
    float** array_to_move_a = nullptr;
    float** array_to_move_b = nullptr;

    std::size_t curr_a_row_size = 0, curr_a_column_size = 0, 
                curr_b_row_size = 0, curr_b_column_size = 0;

    if (argc % 2 == 1) {
        std::cerr << "matrices are mi";
        return 1;
    }

    for (int i = 1; i < argc; ++i) {
        if (i % 2 == 0 && !if_add_or_mult(argv[i])) {
        ...
        if (i == 1) {
            ...
            if (argv[i - 1] == right_add) {
                ...
                get_sum(array_to_move_a, array_to_move_b, curr_a_row_size, curr_a_column_size);
            } else {
